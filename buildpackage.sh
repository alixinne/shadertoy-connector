...
LIBTMPDIR=$(mktemp -d)
cp -r "$LIBDIRECTORY" "$LIBTMPDIR/shadertoy-connector"
cd "$LIBTMPDIR"
...
build_pkg () {
	...
	TARGET_DIRECTORY=$LIBDIRECTORY/../shadertoy-connector-$LIBVERSION$DIR_ARCHSUFFIX
...
build_src () {
	...
	trap 'sudo apt-get -qy autoremove --purge shadertoy-connector-build-deps' EXIT
